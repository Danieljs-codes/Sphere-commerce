...
interface SliderProps extends SliderPrimitiveProps {
	...
	thumbLabels?: string[];
	/**
	 * Optional formatter for displaying values in the slider's outputs (inline and tooltip).
	 * Receives the numeric value and the thumb index, and should return a formatted string.
	 */
					...
					<SliderOutput>
						{({ state }) =>
							props.valueFormatter
								? props.valueFormatter(state.getThumbValue(value), value)
								: state.getThumbValueLabel(value)
						}
					</SliderOutput>
				</Tooltip.Content>
			</Tooltip>
					...
					<SliderOutput className="text-muted-fg text-sm tabular-nums data-[orientation=vertical]:mx-auto data-[orientation=horizontal]:ml-auto">
						{({ state }) =>
							state.values
								.map((_, i) =>
									props.valueFormatter
										? props.valueFormatter(state.getThumbValue(i), i)
